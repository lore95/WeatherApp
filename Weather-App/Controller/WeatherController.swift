...
class WeatherController: ObservableObject {
    ...
    func fetchWeatherData(for place: PlaceSuggestion) async {
        ...
        do {
            ...
            let decodedData = try JSONDecoder().decode(
                ForecastData.self, from: data)
            DispatchQueue.main.async {
                self.forecastData = decodedData
                self.cityName = place.display_name
                self.weatherDescription = self.descriptionForWeatherCode(decodedData.daily.weathercode.first ?? 0)
            }
        } catch {
    ...
    private func filterHourlyData(for data: ForecastData) {
        ...
        self.hourlyToday = hourlyTimes.enumerated().compactMap { index, time in
            if time.hasPrefix(today) {
                let weatherCode =
                    self.forecastData?.daily.weathercode.first ?? 0  // Fallback for icons
                return (
                    time: String(time.suffix(5)), temp: hourlyTemps[index],
                    code: weatherCode
                )
            }
            return nil
    ...
    func formatHour(_ dateString: String) -> String {
        // Split the string on "T" to separate date and time
        let components = dateString.components(separatedBy: "T")

        if components.count > 1 {
            let timePart = components[1]  // Extract "HH:mm"
            let hour = timePart.components(separatedBy: ":").first ?? "00"  // Extract "HH"
            return hour
        }

        return "00"  // Fallback to "00" if the format is unexpected
    }
    ...
    func startAutoRefresh(
        for place: PlaceSuggestion, with interval: TimeInterval
    ) {
        refreshTask?.cancel()  // Cancel any existing task
        refreshTask = Task {
            while !Task.isCancelled {
                await fetchWeatherData(for: place)
